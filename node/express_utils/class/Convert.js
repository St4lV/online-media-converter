...
class Convert {
	...
	async start(new_format) {
		...
		const input_path = `./downloaded/${this.file_name}`;
		const output_path = `./downloaded/${new_file_name}`;
		const args = ["-i", input_path, output_path];
		log.data(`ffmpeg ${args.join(" ")}`)
		const ssh = new SSHCommand("ffmpeg", args);
